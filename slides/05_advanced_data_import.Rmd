...
3. How: with base R functions or a specialised package? Are the data available through an API and if so, is there already a package to make access to it easier?
...
.footnote[
`r kableExtra::text_spec("Extra exercise:", background = "#25496b", color = "#fdf9f6", bold = TRUE)`  If you have worked that out.......
]
--
...
`r kableExtra::text_spec("Extra exercise:", background = "#25496b", color = "#fdf9f6", bold = TRUE)` Use `scan()` to read in the appropriate lines then [tidy](04_tidying_data_and_the_tidyverse.html) the results and name the columns measure_units, for example
```{r echo=FALSE}
...
An API response is usually returned in either:

1. JavaScript Object Notification (JSON) 

--

2. Extensible Markup Language (XML) are the most common answer formats.
...
call1 <- paste(base, endpoint,"?",
               ...
               sep="")
```
https://gtr.ukri.org/search/project?term=virus&fetchSize=25&selectedSortableField=&selectedSortOrder=&fields=&type=&selectedFacets=Y2F0fFJlc2VhcmNoIEdyYW50fHN0cmluZw%3D%3D%2CZnVuZGVyfEJCU1JDfHN0cmluZw%3D%3D%2Cc3RhcnR8MTU3NzgzNjgwMDAwMF8xNjA5NDU5MTk5MDU5fHJhbmdl
--
